...
Date Last Modified: 4/14/21
Description: updates the overall attack and defense of the player
Input parameters: playerStats, playerCurrentInventoryStats, 
	currentInventoryCount
Returns: N/A
**************************************************************/
void updatePlayerStats(int playerStats[3], int playerCurrentInventoryStats[MAX_INVENTORY_SIZE][2], int& currentInventoryCount)
{
	for(int i = 0; i < currentInventoryCount; i++)
	{
		if(playerCurrentInventoryStats[i][1] == 1)
		{
			playerStats[2] = playerStats[2] + playerCurrentInventoryStats[i][0];
		}
		else if(playerCurrentInventoryStats[i][1] == 0)
		{ 
			playerStats[1] = playerStats[1] + playerCurrentInventoryStats[i][0];
		}
	}
}
